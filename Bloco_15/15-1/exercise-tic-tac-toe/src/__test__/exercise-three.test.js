...
winnerSymbols.map(winnerSymbol => {
  describe(`Condições de vitória para o Jogador ${winnerSymbol}`, () => {
    ...
    firstCellsOfLines.map(cellId => {
      test.only(`Alcançar a vitória ao colocar o mesmo simbolo em todas as casas da linha a partir da casa ${cellId}`, () => {
    ...
    firstCellsOfColumns.map(cellId => {
      test.only(`Alcançar a vitória ao colocar o mesmo simbolo em todas as casas da coluna ${cellId}`, () => {
    ...
    test.only('Alcançar a vitória ao colocar o mesmo simbolo na diagonal esquerda para direita (primeira,quinta,nona casa)', () => {
      const {getByTestId, queryByText} = render(<TicTacToe />);
    ...
    test.only('Alcançar a vitória ao colocar o mesmo simbolo na diagonal direita para esquerda (terceira,quinta,sétima casa)', () => {
      const {getByTestId, queryByText} = render(<TicTacToe />);
